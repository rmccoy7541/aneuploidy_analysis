...
aneuploidyByCase <- function(data) {
	data <- data[with(data, order(case, embryoid)),]
	aneuploid_binom <- data.frame()
	for (i in unique(data$case)) {
		subset <- data[data$case == i,]
		aneuploid_0 <- sum(subset$ploidy == TRUE)
		aneuploid_1 <- sum(subset$ploidy == FALSE)
		aneuploid_binom <- rbind(aneuploid_binom, cbind(i, aneuploid_0, aneuploid_1))
	}
	maternal_age <- data.frame(aggregate(data$maternal_age ~ data$case, FUN = mean))
	names(maternal_age) <- c("i", "maternal_age")
	aneuploid_binom <- merge(aneuploid_binom, maternal_age, "i")
	return(aneuploid_binom)
}
...
meioticByCase <- function(data) {
	...
	for (i in unique(data$case)) {
		subset <- data[data$case == i,]
		meiotic <- sum(subset$meiotic == TRUE)
		euploid <- sum(subset$meiotic == FALSE)
		aneuploid_counts <- rbind(aneuploid_counts, cbind(i, euploid, meiotic))
	}
...
mitoticByCase <- function(data) {
	...
	for (i in unique(data$case)) {
		subset <- data[data$case == i,]
		mitotic <- sum(subset$mitotic == TRUE)
		euploid <- sum(subset$mitotic == FALSE)
		aneuploid_counts <- rbind(aneuploid_counts, cbind(i, euploid, mitotic))
	}
	return(aneuploid_counts)
}
