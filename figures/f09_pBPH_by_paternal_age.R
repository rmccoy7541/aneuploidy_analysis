...
c <- ggplot(data = results_all_chroms[results_all_chroms$X5 > 9 | is.na(results_all_chroms$X5),], aes(x = X3, y = X1)) + geom_errorbar(limits, width = 0.5) + geom_line() + geom_point() + xlab("Paternal Age") + ylab("Prop. with Paternal BPH Aneuploidy") + theme(legend.position = "right") + theme_bw() + scale_color_discrete(name = "")
...
meiotic_blastomere <- merge(meiotic_blastomere, paternal_age, "i")
meiotic_te <- merge(meiotic_te, paternal_age, "i")


summary(glm(data = meiotic_blastomere, formula = cbind(euploid, meiotic) ~ paternal_age, family = quasibinomial(link = "logit")))
summary(glm(data = meiotic_te, formula = cbind(euploid, meiotic) ~ paternal_age, family = quasibinomial(link = "logit")))
