...
results <- data.frame(rbind(cbind(seq(1,23,1), p, sqrt((p * (1 - p)) / nrow(data_te)), "Day-5 TE biopsies"), 
cbind(seq(1,23,1), q, sqrt((q * (1 - q)) / nrow(data_blastomere)), "Day-3 blastomeres")))
...
a <- ggplot(data=results, aes(x=chrom, y=prop, fill=sample_type)) + geom_bar(stat="identity", position="dodge") + 
ylab("Prop. aneuploid samples") + xlab("Chromosomes affected") + theme(legend.position="right") + 
labs(fill="Sample type")

b <- ggplot(data=results, aes(x=chrom, y=prop, fill=sample_type)) + geom_bar(stat="identity", position="dodge") + 
theme_bw() + ylab("Proportion Non-euploid Samples") + xlab("Number of Chromosomes Affected") + 
labs(fill="Sample type") + theme(legend.justification=c(0,0), legend.position=c(.25,0.55)) 

difference <- (-1) * ((results[results$sample_type=="Day-3 blastomeres",]$prop - results[results$sample_type == "Day-5 TE biopsies",]$prop) / 
results[results$sample_type=="Day-3 blastomeres",]$prop)

results2 <- data.frame(cbind(1:23, difference))
names(results2) <- c("chrom", "difference")

c <- ggplot(data=results2, aes(x=chrom, y=difference, fill=factor(c(1, rep(2,22))))) + theme_bw() + 
geom_bar(width=0.8, stat="identity", position="identity") + scale_fill_manual(values=c("#66c2a5", "indianred3")) + 
theme(legend.position="none") + ylab("Relative Difference") + xlab("Number of Chromosomes Affected")
