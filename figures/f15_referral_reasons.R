...
referral_reasons <- read.table("~/Desktop/aneuploidy-analysis/data/referral_reasons.csv", sep = ",", header = T)
...
setattr(results_frame$terms, "levels", c("Male factor infertility", "Prev. aneuploid conception", "Previous IVF failure", "Recurr. pregnancy loss", "Translocation carrier", "Unexplained infertility"))
...
p <- ggplot(data = results_frame, aes(x = beta, y = factor(terms), col = factor(terms))) + theme_bw() + geom_point() + geom_errorbarh(limits, height = 0)
p + facet_grid(aneuploidy ~ dev_stage) + ylab('Referral Reason') + xlab('Coefficient') + geom_vline(xintercept = 0, linetype = "dotted") + theme(legend.position="none") + geom_text(aes(x = beta, label = star, vjust = 0.1))
...
p <- ggplot(data = results_frame, aes(x = or, y = factor(terms), col = factor(terms))) + theme_bw() + geom_point() + geom_errorbarh(limits, height = 0)
p + facet_grid(aneuploidy ~ dev_stage) + ylab('Referral Reason') + xlab('Odds Ratio, 95% CI') + geom_vline(xintercept = 1, linetype = "dotted") + theme(legend.position="none") + geom_text(aes(x = or, label = star, vjust = 0.1))


q <- ggplot(data = results_frame[terms == "Translocation carrier" | terms == "Previous IVF failure" | terms == "Recurr. pregnancy loss"], aes(x = or, y = factor(terms), col = factor(terms))) + theme_bw() + geom_point() + geom_errorbarh(limits, height = 0)
q + facet_grid(aneuploidy ~ dev_stage) + ylab('Referral Reason') + xlab('Odds Ratio, 95% CI') + geom_vline(xintercept = 1, linetype = "dotted") + theme(legend.position="none") + geom_text(aes(x = or, label = star, vjust = 0.1))
