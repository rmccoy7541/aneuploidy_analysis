...
maternal_age <- cbind(case = data[!duplicated(data$case),]$case, maternal_age = data[!duplicated(data$case),]$maternal_age)
aneuploid_binom <- merge(aneuploid_binom, maternal_age, "case")

g0 <- glm(data = aneuploid_binom, formula = cbind(aneuploid, euploid) ~ maternal_age, 
family = quasibinomial(link = "logit"))

g1 <- glm(data = aneuploid_binom, formula = cbind(aneuploid, euploid) ~ maternal_age + I(maternal_age^2), 
family = quasibinomial(link = "logit"))

anova(g0, g1, test="F")

g2 <- glm(data = aneuploid_binom, formula = cbind(aneuploid, euploid) ~ maternal_age + I(maternal_age^2) + 
I(maternal_age^3), family = quasibinomial(link = "logit"))

anova(g1, g2, test="F")

# Fit model to data
g3 <- glm(data = aneuploid_binom, formula = cbind(aneuploid, euploid) ~ maternal_age + I(maternal_age^2) + 
...
ggplot(data = results_all_chroms[results_all_chroms$X5 > 9 | is.na(results_all_chroms$X5),], 
aes(x = X3, y = X1, col = factor(X4))) + geom_errorbar(limits, width = 0.5) + geom_line() + geom_point() + 
xlab("Maternal Age") + ylab("Proportion Aneuploid Biopsies") + theme(legend.position="right") + theme_bw() + 
...
g4 <- glm(data = aneuploid_binom, formula = cbind(aneuploid, euploid) ~ maternal_age + I(maternal_age^2) + 
I(maternal_age^3) + donor, family = quasibinomial(link = "logit"))
...
g3 <- glm(data = aneuploid_binom, formula = cbind(aneuploid, euploid) ~ maternal_age + I(maternal_age^2) + 
donor, family = quasibinomial(link = "logit"))
...
aes(x = X3, y = X1, col = factor(X4))) + geom_errorbar(limits, width = 0.5) + geom_line() + geom_point() + 
xlab("Maternal Age") + ylab("Proportion Aneuploid") + theme(legend.position = "right") + theme_bw() + 
scale_color_discrete(name = "")
