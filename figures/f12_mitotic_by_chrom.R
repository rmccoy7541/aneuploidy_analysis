...
c <- ggplot(data = data_filtered[data_filtered$meiotic == FALSE & data_filtered$mitotic == TRUE & data_filtered$chroms_affected > 0,], aes(x = chroms_affected, fill = factor(sample_type))) + geom_histogram(binwidth = 1, position = "dodge") + xlab('Total Aneuploid Chromosomes') + ylab('Number of Samples') + theme(legend.position="none")
